/* MANTENEDOR DE USUARIOS (vista administrador)

   Reglas pedidas en el Anexo 1:
   - RUN:        requerido, sin puntos ni guion (ej: 19011022K), entre 7 y 9
                 caracteres, y además se valida el dígito verificador.
   - Nombre:     requerido, máximo 50.
   - Apellidos:  requerido, máximo 100.
   - Correo:     requerido, máximo 100, solo @duoc.cl, @profesor.duoc.cl y @gmail.com.
   - Fecha nacimiento: opcional.
   - Tipo usuario: requerido (Administrador, Cliente o Vendedor).
   - Región y comuna: al cambiar la región cambian las comunas.
   - Dirección:  requerida, máximo 300.

   Los usuarios quedan guardados en localStorage con la clave "usuarios_admin".
   Es una clave distinta a la que usa cuenta.js ("usuarios_sonido"), para no
   pisar los registros que hacen los clientes desde la tienda. */

const CLAVE_USUARIOS = "usuarios_admin";

let runEditando = null; // null = estamos creando, con texto = estamos editando
...
function obtenerUsuariosAdmin() {
    ...
    // La primera vez dejamos dos usuarios de ejemplo para que la tabla no salga vacía.
    return [
        {
            run: "190110222",
            nombre: "Camila",
...
/* ---------- Validación del RUN (módulo 11) ----------
   Se recorre el RUN de derecha a izquierda multiplicando por 2,3,4,5,6,7
   y volviendo a 2. Con eso se calcula el dígito verificador esperado. */
...
function validarRun() {
    ...
    // No se puede repetir (salvo que sea el mismo que estamos editando).
...
// Cada vez que cambia la región, se vuelven a cargar las comunas.
