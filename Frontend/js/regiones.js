/* REGIONES Y COMUNAS
   Arreglo simple para los select dependientes del mantenedor de usuarios:
   al elegir una región, se cargan solo sus comunas.
   (Es una selección reducida, no están todas las comunas del país.) */

const regiones = [
    {
        nombre: "Región Metropolitana de Santiago",
        comunas: ["Santiago", "Providencia", "Las Condes", "Maipú", "Puente Alto", "La Florida", "Colina", "Ñuñoa"]
    },
    {
        nombre: "Región de Valparaíso",
        comunas: ["Valparaíso", "Viña del Mar", "Quilpué", "Villa Alemana", "San Antonio", "Quillota"]
    },
    {
        nombre: "Región del Biobío",
        comunas: ["Concepción", "Talcahuano", "San Pedro de la Paz", "Chiguayante", "Los Ángeles", "Coronel"]
    },
    {
        nombre: "Región de La Araucanía",
        comunas: ["Temuco", "Padre Las Casas", "Villarrica", "Pucón", "Angol", "Victoria"]
    },
    {
        nombre: "Región de Ñuble",
        comunas: ["Chillán", "Chillán Viejo", "San Carlos", "Bulnes", "Quirihue"]
    },
    {
        nombre: "Región de Los Lagos",
        comunas: ["Puerto Montt", "Osorno", "Castro", "Ancud", "Puerto Varas"]
    },
    {
        nombre: "Región de Antofagasta",
        comunas: ["Antofagasta", "Calama", "Tocopilla", "Mejillones"]
    },
    {
        nombre: "Región del Maule",
        comunas: ["Talca", "Curicó", "Linares", "Longaví", "Constitución"]
    }
];
