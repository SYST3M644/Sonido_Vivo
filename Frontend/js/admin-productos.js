/* MANTENEDOR DE PRODUCTOS (vista administrador)

   Reglas pedidas en el Anexo 1:
   - Código:       requerido, texto, mínimo 3, sin máximo, no se puede repetir.
   - Nombre:       requerido, máximo 100.
   - Descripción:  opcional, máximo 500.
   - Precio:       requerido, mínimo 0 (0 = producto FREE), acepta decimales.
   - Stock:        requerido, mínimo 0, solo enteros.
   - Stock crítico: opcional, mínimo 0, solo enteros.
   - Categoría:    requerida, se elige desde un select.
   - Imagen:       opcional.

   Los productos que se crean o editan aquí quedan guardados en localStorage
   (clave "productos_sonido"), porque todavía no tenemos base de datos.
   El arreglo original vive en productos.js y no se toca. */

const CLAVE_PRODUCTOS = "productos_sonido";

let codigoEditando = null; // null = estamos creando, con texto = estamos editando
...
function obtenerProductosAdmin() {
    ...
    // La primera vez copiamos el arreglo original de productos.js.
...
function validarCodigo() {
    ...
    // Si estamos creando, el código no puede existir ya.
...
function validarPrecio() {
    ...
    if (numero === 0) {
        // No es un error, es un aviso: el producto queda como FREE.
...
function validarCritico() {
    ...
    if (texto === "") {
        return marcarCorrecto("p-critico"); // es opcional
    }
...
function guardarProducto(evento) {
    ...
    if (codigoEditando === null) {
        ...
        // Reemplazamos el producto que estábamos editando.
...
function editarProducto(codigo) {
    ...
    document.getElementById("p-codigo").disabled = true; // el código identifica al producto
...
function mostrarTabla() {
    ...
    // Conectamos los botones que acabamos de dibujar.
...
/* ---------- Llenar el select de categorías y la lista de marcas ---------- */
